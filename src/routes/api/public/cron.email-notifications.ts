...
import {
  ...
  enqueueEmailDueSoonNotifications,
} from "@/lib/email/notifications.server";

/**
 * Cron sweep: process the email notification queue and enqueue
 ...
 */
export const Route = createFileRoute("/api/public/cron/email-notifications")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          const [queueResult, dueSoonResult] = await Promise.all([
            processEmailNotificationQueue(50),
            enqueueEmailDueSoonNotifications(2),
          ]);
